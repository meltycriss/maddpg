...
class Experience(object):
    ...
    def __init__(self, conf):
        ...
        assert 1.*self.size/self.partition_num < self.learn_start
    ...
    def sample(self, global_step):
        ...
        dist_index = int(math.floor(float(self.record_size) / self.size * self.partition_num))
        # issue 1 by @camigord
        ...
        for n in range(1, self.batch_size + 1):
            l = distribution['strata_ends'][n]
            r = distribution['strata_ends'][n+1]
            if l<r:
                l += 1
